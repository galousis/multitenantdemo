...
use App\Infrastructure\Media\Exceptions\UrlCannotBeDetermined;
...
class LocalUrlGenerator extends BaseUrlGenerator
{
    ...
    {
        if ($diskUrl = $this->config->get("filesystems.disks.{$this->media->getDiskDriverName()}.url")) {
    ...
    {
        $diskRootPath = $this->config->get("filesystems.disks.{$this->media->getDiskDriverName()}.root");
