...
namespace App\Infrastructure\Media\Filesystem;
...
use App\Infrastructure\Media\Events\MediaHasBeenAdded;
...
class DefaultFilesystem implements Filesystem
{
    ...
    {
        $sourceFile = $this->getMediaDirectory($media).'/'.$media->getFileName();

        touch($targetFile);

        $stream = $this->filesystem->disk($media->getDiskDriverName())->readStream($sourceFile);
    ...
    public function removeFiles(Media $media)
    {
        ...
        collect([$mediaDirectory, $conversionsDirectory])
            ->each(function ($directory) use ($media) {
                $this->filesystem->disk($media->getDiskDriverName())->deleteDirectory($directory);
            });
    }
    ...
    public function renameFile(Media $media, string $oldName)
    {
        ...
        $newFile = $this->getMediaDirectory($media).'/'.$media->getFileName();

        $this->filesystem->disk($media->getDiskDriverName())->move($oldFile, $newFile);
    }
    ...
    public function getMediaDirectory(Media $media, bool $conversion = false) : string
    {
        ...
        if (! in_array($media->getDiskDriverName(), ['s3'], true)) {
            $this->filesystem->disk($media->getDiskDriverName())->makeDirectory($directory);
        }
